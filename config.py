...
REGEX_PATTERNS = {
    ...
    'business_purpose': r'Gegenstand des Unternehmens:\s*(.*?)(?=\n\d+\.)',
    'capital': r'Grund- oder Stammkapital:\s*([^\n]+)',
    'representation_rules': r'Allgemeine Vertretungsregelung:\s*(.*?)(?=\nb\))',
    'management_section': r'Geschäftsführer:\s*(.*?)(?=\n\d+\.)',
    'managers': r'([^,]+),\s*([^,]+),\s*([^,]+),\s*\*(\d{2}\.\d{2}\.\d{4})',
    'prokura_section': r'Prokura:\s*(.*?)(?=\n\d+\.)',
    'prokura_holders': r'([^,]+),\s*([^,]+),\s*([^,]+),\s*\*(\d{2}\.\d{2}\.\d{4})',
