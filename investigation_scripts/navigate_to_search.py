        ...
        try:
            ...
            for link in browser.links():
                if link.text and link_text.lower() in link.text.lower():
                    print(f"  Found matching link: '{link.text}' -> {link.url}")
                    found_link = True
                    
                    if link.url != '#':
                        try:
                            response = browser.follow_link(link)
                            print(f"  ✓ Successfully navigated to: {browser.geturl()}")
...
def try_constructed_urls_with_session(browser, session_id):
    ...
    target_urls = [
        f"https://www.handelsregister.de/rp_web/normalesuche.xhtml{session_param}",
        f"https://www.handelsregister.de/rp_web/erweitertesuche.xhtml{session_param}?cid=2"
    ]
    ...
    for url in target_urls:
        try:
            ...
            # Analyze forms
            forms_info = analyze_current_page_forms(browser)
            search_forms = [f for f in forms_info if any(c['is_search_field'] for c in f['controls'])]
            
            print(f"Forms with search fields: {len(search_forms)}")
            for form in search_forms:
                search_controls = [c for c in form['controls'] if c['is_search_field']]
                print(f"  Form '{form['name']}': {len(search_controls)} search fields")
